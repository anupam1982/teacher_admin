  ...
  try {
    ...
    for (var i = 0; i < students.length; i++) {
      if (existing_students.indexOf(students[i]) < 0) {
        ...
        let suspendStatus = await studentsRepository.getStudentStatus(students[i]);
        if(suspendStatus && suspendStatus[0] && suspendStatus[0].status === 'false'){
          await studentsRepository.updateStudentStatus(students[i], 'true');
        }
      }
    }
...
exports.deregister = async function (req, res) {
  ...
  await teachersRepository.deleteTeacherRecord(teacher);
}